...
async function fetchHotelChannels(hotelName, city, serpApiKey, checkIn, checkOut) {
...
serve(async (req) => {
  ...
  try {
    const { hotels, city, checkIn, checkOut } = await req.json(); // hotels: string[] of hotel names, first = your property
    if (!Array.isArray(hotels) || hotels.length === 0) return json({ unavailable: true, reason: "No hotels provided", hotels: [] });

    const results = await Promise.all(hotels.map((h) => fetchHotelChannels(h, city, serpApiKey, checkIn, checkOut)));
    return json({ unavailable: false, hotels: results });
  } catch (e) {
